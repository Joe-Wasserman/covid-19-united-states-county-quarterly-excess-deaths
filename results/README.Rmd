...
walk(
  c(
    file.path(here::here(), "code", "augment_glmmTMB2.R"),
...
Compare model performance indices for all models.
Although many models perform similarly for these metrics, when taken together and with a focus on AIC and BIC, model 5 (counties nested within county sets nested within states) and model 9 (counties nested within county sets nested within states nested within census regions) appear to be the top two contenders among LMMs.
These two models were also estimated with an AR1 autoregressive term via glmmTMB.
...
## DEV: Model summary for all glmmTMB output
```{r glmmtmb summaries, include=TRUE, message=TRUE}
model_out %>%
  walk(
    .,
    ~ .x[[1]] %>% 
...
Compare mean squared error (MSE) of model-predicted death rates against observed death rates in January and February, 2020.
Because the COVID-19 pandemic only began partway through March, 2020, we can evaluate model performance by examining concordance of predicted and observed deaths in these two months.
The two autoregressive models outperform the non-autoregressive LMMs by this measure.
...
Based on these results, model 10 (model 5 with an additional AR1 autoregressive term) was selected as the final model.
...
united_states_county_monthly_model[[1]] %>% 
  broom.mixed::tidy() %>% 
    kable(
    format = "pipe"
  )
